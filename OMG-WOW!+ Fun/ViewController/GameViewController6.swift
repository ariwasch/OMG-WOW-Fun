...
class GameViewController6: UIViewController, GADInterstitialDelegate, GADRewardedAdDelegate {
    
    
    var gameScene : MyScene6?
    ...
    override func viewDidLoad() {
        ...
        currentLevel = 90
        loadAds()
        allStrings = block.allStrings6
        view.layoutIfNeeded()
        presentGameScene()
        initializationOfGameVariable()
        hideGameComponents()
        initializeGameSwipeAction()
        if(defaults.integer(forKey: "game6level") != 90 && (defaults.integer(forKey: "game6level") != 0)){
            skip(to: defaults.integer(forKey: "game6level"))
        }
        reloadBalance()
    ...
    func presentGameScene()
    {
        if let view = self.view as! SKView? {
            if let scene = SKScene(fileNamed: "MyScene6") as? MyScene6 {
    ...
    func wordInWord(string: String) -> Bool{
        ...
        if((string == "FEEL" && currentLevel == 99)){
            isDuplicate = false
        }
        return isDuplicate
    }
    ...
    func duplicate(string: String) -> Bool{
        ...
        print(currentLevel)
        if(string == "WHEN"  || string == "FEEL" || string == "GENEROUS" || string == "SAID" || string == "OTHERS"){
            isDuplicate = true
        }
        return isDuplicate
    }
    func duplicateHelper(string: String) -> Int{
        var num = 0
        if(string == "WHEN" && currentLevel == 100){
            num = 99 - previous
        }else if(string == "WHEN" && currentLevel == 99){
            num = 98 - previous
        }
        if(string == "GENEROUS" && currentLevel == 99){
            num = 98 - previous
        }else if(string == "GENEROUS" && currentLevel == 95){
            num = 94 - previous
        }
        if(string == "FEEL" && currentLevel == 93){
            num = 92 - previous
        }else if(string == "FEEL" && currentLevel == 102){
            num = 101 - previous
        }
        if(string == "SAID" && currentLevel == 91){
            num = 90 - previous
        }else if(string == "SAID" && currentLevel == 102){
            num = 101 - previous
        }
        if(string == "OTHERS" && currentLevel == 91){
            num = 90 - previous
        }else if(string == "OTHERS" && currentLevel == 101){
            num = 100 - previous
        }else if(string == "OTHERS" && currentLevel == 103){
            num = 102 - previous
        }
    ...
    func initializeNextLevel(level: Int, title: String, popTitle: String, popBody: String)
    {
        ...
        if(enableEndPop){
            ...
            defaults.set(90, forKey: "game6level")
            defaults.set(false, forKey: "startview")
            performSegue(withIdentifier: "levelselect6", sender: nil)
        }else{
        ...
        self.defaults.set(level, forKey: "game6level")
        ...
        if let touch = touches.randomElement(), gameScene != nil
        {
            ...
            if touchedNode == gameBack
            {
                ...
                if(currentLevel > previous+1){
                    ...
                    defaults.set(currentLevel-1, forKey: "game6level")
                    viewDidLoad()
                }else{
                    defaults.set(false, forKey: "startview")
                    performSegue(withIdentifier: "levelselect6", sender: nil)
